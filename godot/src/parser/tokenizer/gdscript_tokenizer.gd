	...
	while cur_index < input_string.length():
		...
		for tokenizer_module in tokenizer_modules:
			...
			if cur_token_length != -1:
				...
				print("Module chosen: ", tokenizer_module.get_script().get_global_name())
				print("Token string: ", input_string.substr(cur_index, cur_token_length))
				break
		...
		if token != null:
			...
			# increment line number if we pass a new line.
			if token.string_rep.ends_with("\n"):
				line_num += 1
